...
numberOfRolls = 0
currentIterationRollsRemoved = None

with open(input_file, "r") as file:
    lines = [list(line.strip()) for line in file]
    numberOfLines = len(lines)
    while currentIterationRollsRemoved != 0:
        currentIterationRollsRemoved = 0
        for lineIndex, line in enumerate(lines):
            rollsRow = line
            previousRollsRow = lines[lineIndex - 1] if lineIndex > 0 else None
            nextRollsRow = lines[lineIndex + 1] if lineIndex < numberOfLines - 1 else None
            for placeIndex, place in enumerate(rollsRow):
                ...
                if place == "@":
                    ...
                    if tempNumberOfRolls < 4:
                        lines[lineIndex][placeIndex] = "."
                        currentIterationRollsRemoved += 1
                        numberOfRolls += 1
